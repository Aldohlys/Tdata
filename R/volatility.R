...
fitHAR <- function(sym, lookback_days = 400, source = c("yahoo", "ibkr"),
  ...
  # Prepare HAR model data - always train for 1-day ahead prediction
  # Multi-day forecasts are generated by iterating the 1-day model
  model_data <- prepare_har_data(prices, calc = calc, n = n, forecast_horizon = 1)
  ...
  # Calculate forecast accuracy metrics (on variance)
  rmse <- sqrt(mean((test_data$rv_next - predictions)^2))
  mae <- mean(abs(test_data$rv_next - predictions))

  # Calculate MAE directly on annualized volatility for meaningful error bounds
  actual_vol <- sqrt(test_data$rv_next * 252)
  predicted_vol <- sqrt(predictions * 252)
  mae_vol <- mean(abs(actual_vol - predicted_vol))


  # Direct multi-horizon forecasting: train separate target for each horizon
  # This avoids compounding errors from iterative forecasting
  latest_rv_daily <- utils::tail(model_data$rv_daily, 1)
  latest_rv_weekly <- utils::tail(model_data$rv_weekly, 1)
  latest_rv_monthly <- utils::tail(model_data$rv_monthly, 1)

  latest_data_df <- data.frame(
    rv_daily = latest_rv_daily,
    rv_weekly = latest_rv_weekly,
    rv_monthly = latest_rv_monthly
  )

  # For each horizon h, create target as rv at t+h and fit model
  forecast_rv_vec <- numeric(forecast_horizon)

  for (h in seq_len(forecast_horizon)) {
    # Create target for horizon h: lead by h days from model_data
    model_data_h <- model_data
    model_data_h$rv_target <- dplyr::lead(model_data$rv_daily, h)
    model_data_h <- stats::na.omit(model_data_h)

    # Fit model for horizon h
    har_model_h <- stats::lm(rv_target ~ rv_daily + rv_weekly + rv_monthly, data = model_data_h)

    # Predict using current values
    forecast_rv_vec[h] <- as.numeric(stats::predict(har_model_h, newdata = latest_data_df))
  }

  # Convert to annualized volatility
  forecast_vol_annualized <- sqrt(forecast_rv_vec * 252)

  # Calculate blended forecast: weighted toward current vol for near-term tradability
  # Weight decreases HAR influence: 70% current + 30% HAR adjustment
  current_vol <- sqrt(latest_rv_daily * 252)
  blended_forecast <- 0.7 * current_vol + 0.3 * forecast_vol_annualized

  logger::log_info("HAR forecast for {sym} ({forecast_horizon} days): {paste(round(blended_forecast, 4), collapse=', ')} (blended)",
                   namespace = "Tdata")

  # Calculate current realized volatility at different windows for comparison
  # These are the annualized volatilities based on the RV components
  current_rv_daily <- utils::tail(model_data$rv_daily, 1)
  current_rv_weekly <- utils::tail(model_data$rv_weekly, 1)
  current_rv_monthly <- utils::tail(model_data$rv_monthly, 1)

  current_vol_n <- sqrt(current_rv_daily * 252)       # Current vol at n-day window
  current_vol_5d <- sqrt(current_rv_weekly * 252)     # 5-day rolling avg
  current_vol_22d <- sqrt(current_rv_monthly * 252)   # 22-day rolling avg

  # Store latest RV components for reference
  ...
  return(list(
    ...
    forecast = as.numeric(blended_forecast),        # Tradeable: 70% current + 30% HAR
    forecast_raw = as.numeric(forecast_vol_annualized),  # Pure HAR model forecast
    forecast_error = as.numeric(mae_vol),           # Typical error in vol points (± range)
    current_vol_n = as.numeric(current_vol_n),
    current_vol_5d = as.numeric(current_vol_5d),
    current_vol_22d = as.numeric(current_vol_22d),
    rmse = rmse,
    mae = mae,
...
#'
#' Creates an interactive plotly chart comparing actual vs predicted volatilities
#' for the test period. Hover over points to see values.
#'
...
plotHAR <- function(har_result, main = NULL) {
  ...
  if (is.null(main)) {
    main <- paste0(har_result$symbol, " - Actual vs Predicted Volatility")
  }

  # Convert to annualized volatility percentages
  actual_vol_pct <- sqrt(har_result$test_actual * 252) * 100
  predicted_vol_pct <- sqrt(har_result$test_predicted * 252) * 100
  dates <- har_result$test_dates

  # Create data frame for plotly
  plot_data <- data.frame(
    date = dates,
    actual = actual_vol_pct,
    predicted = predicted_vol_pct
  )

  # Create plotly chart
  p <- plotly::plot_ly(plot_data, x = ~date) |>
    plotly::add_lines(y = ~actual, name = "Actual",
                      line = list(color = "blue"),
                      hovertemplate = "Date: %{x}<br>Actual: %{y:.1f}%<extra></extra>") |>
    plotly::add_lines(y = ~predicted, name = "Predicted",
                      line = list(color = "red"),
                      hovertemplate = "Date: %{x}<br>Predicted: %{y:.1f}%<extra></extra>") |>
    plotly::layout(
      title = main,
      xaxis = list(title = "Date"),
      yaxis = list(title = "Volatility (%)", ticksuffix = "%"),
      hovermode = "x unified",
